...
| 密钥 | 获取方式 |
|------|---------|
| 千问 API Key | 注册 [阿里云百炼](https://bailian.console.aliyun.com/)，创建 API Key（格式 `sk-xxx`） |
| 钉钉 Webhook | 钉钉群 → 群设置 → 机器人 → 自定义机器人 → 复制 Webhook 地址，安全设置选"自定义关键词" |
...
#### 5. 启动 AI 助手（可选，需要安装 Node.js 和 Claude Code CLI）

```bash
# 安装 Claude Code CLI（AI 助手的后端引擎）
